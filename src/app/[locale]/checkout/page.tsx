    ...
    <div className="mx-auto max-w-5xl px-4 py-12 lg:px-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 grid min-w-0 gap-10 lg:grid-cols-[minmax(0,1fr)_min(100%,340px)]">
        <div className="space-y-4">
          ...
          />
          {method === 'pickup' && (
            <Input label={t('pickupLocation')} {...register('pickupLocationName')} error={errors.pickupLocationName?.message} />
          )}
          {method === 'shipping' && (
            <>
              <Input label="Name" {...register('shippingAddress.name')} error={errors.shippingAddress?.name?.message} />
              <Input label="Street" {...register('shippingAddress.street')} error={errors.shippingAddress?.street?.message} />
              <div className="grid gap-4 sm:grid-cols-2">
                <Input label="City" {...register('shippingAddress.city')} error={errors.shippingAddress?.city?.message} />
                <Input label="State" {...register('shippingAddress.state')} error={errors.shippingAddress?.state?.message} />
              </div>
              <Input label="ZIP" {...register('shippingAddress.zip')} error={errors.shippingAddress?.zip?.message} />
            </>
          )}
        ...
        <aside className="rounded-2xl border border-grey/15 bg-cream p-6">
          ...
          <div className="mt-4"><CartSummary /></div>
        </aside>
      </form>
    </div>
  );
