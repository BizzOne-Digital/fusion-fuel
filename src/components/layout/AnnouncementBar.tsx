...
export function AnnouncementBarComponent({ announcement, locale }: AnnouncementBarProps) {
  ...
  const content = announcement.link ? (
    <Link href={parseAppLinkHref(announcement.link)} className="underline underline-offset-2">
      {message}
    </Link>
  ) : (
    message
  ...
  return (
    <div
      className="relative z-40 w-full overflow-x-hidden px-4 py-2 text-center text-sm font-medium"
      style={{
